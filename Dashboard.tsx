...
const Dashboard: React.FC<DashboardProps> = (props) => {
  ...
  const getRoleSpecificTabName = () => {
    switch(currentUser.role) {
      ...
      case Role.VERIFIER: return 'Verification Tasks';
      case Role.APPROVER: return 'Approval Tasks';
  ...
  const renderRoleSpecificContent = () => {
    switch (currentUser.role) {
      ...
      case Role.VERIFIER:
        const verifierExpenses = expenses.filter(e => 
            e.status === Status.PENDING_VERIFICATION || 
            e.history.some(h => h.actorId === currentUser.id)
        );
        return (
          <VerifierDashboard
            currentUser={currentUser}
            expenses={verifierExpenses}
            categories={categories}
            onUpdateExpenseStatus={onUpdateExpenseStatus}
          />
        );
      case Role.APPROVER:
        const approverExpenses = expenses.filter(e => 
            e.status === Status.PENDING_APPROVAL ||
            e.history.some(h => h.actorId === currentUser.id)
        );
        return (
          <ApproverDashboard
            currentUser={currentUser}
            expenses={approverExpenses}
            categories={categories}
            onUpdateExpenseStatus={onUpdateExpenseStatus}
          />
        );
