...
const VerifierDashboard: React.FC<VerifierDashboardProps> = ({ expenses, categories, onUpdateExpenseStatus }) => {
  ...
  const filteredExpenses = expenses.filter(expense => {
    ...
    if (statusFilter === 'Pending') {
        statusMatch = expense.status === Status.PENDING_VERIFICATION;
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold tracking-tight text-gray-900">Verification Tasks</h2>
      <p className="mt-1 text-sm text-gray-600">Review, verify, and track expense requests.</p>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
           <div>
            ...
            >
              ...
              <option value={Status.PENDING_APPROVAL}>{Status.PENDING_APPROVAL}</option>
